...
college_share <- chile |>
  ...
  mutate(college = as.integer(EDCAT7 >= 5)) |>
  group_by(cohort_decade, round, survey_year) |>
  summarise(
    college_share = weighted.mean(college, SPFWT0, na.rm = TRUE),
    n_obs = n(),
    .groups = "drop"
  ) |>
  arrange(cohort_decade, round)

college_share_matched <- college_share |>
  select(cohort_decade, round, college_share, n_obs) |>
  pivot_wider(
    names_from = round,
    values_from = c(college_share, n_obs),
    names_sep = "_r"
  ) |>
  filter(!is.na(college_share_r1), !is.na(college_share_r2)) |>
